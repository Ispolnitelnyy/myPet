...
необходимо произвести рефакторинг: крточки профиля `<ProfileCard/>` и страницы профиля `<ProfilePage/>`, а именно вынести всю логику на уровень страницы `<ProfilePage/>` и в качестве пропсов передавать данные во вложенный компонет чем и является `<ProfileCard/>`
так будет праввильно с точки зрения структурирования логики в приложении, поскольку карточек на странице может быть несколько
