...
```
   ...
   нашел решение в сторонней либе: `typed-scss-modules`, она под каждым css, scss файлом создает `index.module.d.scss` и вписывает туда доступные типы, что решает проблему
...
```
Важно
В массиве лоудеров `babelLoader` должен быть первее в индексе чем `typescriptLoader`
```
...
оставлю пока так, потом разберусь, в любом случае для исправления еслинт больше не доступен,
только подчеркивания неправильного синтаксиса и проблем (для чего собственно он и нужен),
...
убрал пример переводчика на `<mainPage/>` - комит: [jest b305355] refactor theme colors, and translator on main page
```
...
```
устанавливаем `presset` в babel для ts: `npm install --save-dev @babel/preset-typescript`
прописываем `presset` в `babel.config.json`: `"@babel/preset-typescript"`
(работает без него)
устанавливаем `presset` бэйбла для react: `npm install --save-dev react-test-renderer`
прописываем `presset` в `babel.config.json` `["@babel/preset-react",{"runtime":"automatic"}] `
(работает без него)
...
в корне проекта появляется папка `.storybook` (убираем точку, переносим ее в папук `config` где лежит webpack, в файлe `main.ts` меняем путь, на выход на 1 дирректорию `stories: ["../../src/**/*.stories.@(js|jsx|mjs|ts|tsx)"]` )
...
непосредсвенно в саму сторю причем не важно где будет импорт стилей, в сторе или в декораторе
для перфоманса оставлю импорт стилей в декораторе
Ключевое - preview.tsx вообще бесполезный,
ни декоратор, ни стили туда импортировать не надо! (почти каждое решение я находил именно с импортом в preview.tsx)
...
скриншотные тесты не запускаются в пайплайне, пока закоментил их, не хочу тратить время, займусь ими при подключении `Docker`
...
есть проблема. что Chrome который использует локи не успевает за 10 секунд заскринить некоторые компоненты:
...
`node_modules\@loki\browser\src\await-selector-present.js` нашел где устанавливается значение
...
Необходимо сделать состояние: `const [isClosing, setIsClosing] = useState(false)` и нужен референс в котором будем хранить таймаут: `const timerRef = useRef<ReturnType<typeof setTimeout>>()` в момент когда мы будем закрывать окно, мы будем вешать `setTimeout`, этот `setTimeout` будем получать в `timerRef.current = setTimeout()` для чтобы можно было его отчистить ретерном колбэка из useEffect. Внутри таймаута вызываем `OnClose()` который и закроет модалку. Теперь необходимо изменять состояние, когда нажимаем на  
закрытие, мы делаем `setIsClosing(true)` а потом нам надо сделать его со значением `false` но уже в `setTimeout`. В момент когда `IsClosing === true` мы сможем обратиться к классу и навесить на него необходимые стили и анимации. После закрытия сохранится последнее состояние: `IsClosing === true`, и это можно пофиксить следующим путем: `setIsClosing(false)` по завершению `setTimeout`.  
...
```
произвел огромный рефактор, не оставляю надежды починить скриншотные тесты и билд сторибука
...
```
столкнулся с ошибкой линтера: oбычный `useDispatch()` не умет работать с асинхронными экшенами,
для него необходимо указывать тип во время объявления хука: `useDispatch<AppDispatch>()`.
Документация предложила сделать кастомный универсальный хук: `useAppDispatch`,
который будет являться оберткой `useDispatch<AppDispatch>()` (диспача с типом),
